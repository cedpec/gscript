...
if (typeof require !== 'function') {
  ...
  function isIgnoredByClasp(p) {
    ...
    for (const pattern of claspIgnores) {
      ...
      if (pattern.endsWith('/')) {
        if (rel === pattern.slice(0, -1) || rel.startsWith(pattern.slice(0, -1) + '/')) return true;
      }
  ...
  for (const ff of forbiddenFiles) {
    ...
    if (fs.existsSync(p) && !isIgnoredByClasp(p)) found.push(`Forbidden file present: ${ff}`);
  }
  ...
  walk(root, (file) => {
    ...
    forbiddenPatterns.forEach((pat) => {
      if (pat.test(content)) found.push(`Pattern ${pat} found in ${path.relative(root, file)}`);
    });
  });
